...
function getScoreTier(score: number): { color: string; gradient: string; label: string } {
  if (score >= 30) return { color: 'text-yellow-600', gradient: 'from-yellow-400 to-amber-500', label: 'Legendary' }
  if (score >= 20) return { color: 'text-amber-600', gradient: 'from-amber-400 to-orange-500', label: 'Outstanding' }
  if (score >= 12) return { color: 'text-orange-600', gradient: 'from-orange-400 to-red-400', label: 'Excellent' }
  if (score >= 6)  return { color: 'text-rose-600', gradient: 'from-rose-400 to-pink-500', label: 'Great' }
  if (score >= 3)  return { color: 'text-violet-600', gradient: 'from-violet-400 to-purple-500', label: 'Rising' }
  ...
  return (
    <div className={`flex flex-col items-end ${s.gap} relative`}>
      ...
      <div className="flex items-center gap-1">
        <span className={`font-extrabold ${tier.color} ${s.score} leading-none`}>
          {score}
        </span>
        <button
