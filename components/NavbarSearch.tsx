...
export function NavbarSearch() {
  ...
  const hasResults = results && (results.restaurants.length > 0 || results.categories.length > 0 || results.combos?.length > 0)
  const noResults = results && results.restaurants.length === 0 && results.categories.length === 0 && (!results.combos || results.combos.length === 0) && query.trim().length >= 2
  ...
  const dropdown = open && (hasResults || noResults) ? (
    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-amber-100 rounded-xl shadow-lg overflow-hidden z-50 max-h-[70vh] overflow-y-auto">
      {results!.combos?.length > 0 && (
        <div>
          <div className="px-3 pt-2.5 pb-1 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Category + Location</div>
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 bg-white">
            ...
            <div className="overflow-y-auto max-h-[calc(100vh-3.5rem)]">
              {results?.combos && results.combos.length > 0 && (
                <div>
                  <div className="px-4 pt-3 pb-1 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Category + Location</div>
                  {results.combos.map(c => (
                    <button
                      key={`${c.categorySlug}-${c.city}-${c.state}`}
                      onClick={() => navigate(
